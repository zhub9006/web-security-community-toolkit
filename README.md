...
### Clinical Trials
- Searched via **ClinicalTrials.gov API v2** (`term="privacy"`; filtered `COMPLETED`, `ACTIVE_NOT_RECRUITING`, `RECRUITING`)
- Individual studies retrieved per NCT ID for full summaries — no fabricated IDs
- All API calls logged 2026-07-13; six studies found, two directly verified in this session
- Full comparison in [docs/trial-privacy-matrix.md](./docs/trial-privacy-matrix.md)

### Portland Venues
- Geocoded via **OpenStreetMap Nominatim API** for each venue name
- Distances computed as straight-line from Pioneer Courthouse Square (45.5189, −122.6793)
- Categories filtered: `community_centre`, `conference_centre`, `museum`, `library`, `university`, `office`, `shop`
- Full guide in [docs/portland-venues.md](./docs/portland-venues.md)